    ...
    public Double[] execute(Exam exam, Student currentStudent) {
        ...
        exam.getStudents().forEach(student -> {
            if (student != currentStudent) {
                int score = 0;
                String studentCode = String.join("\n",student.getFilesLines());
                LinkedList<DiffPatchMatch.Diff> diffs = diffPatchMatch.diff_main(studentCode,String.join("\n",currentStudent.getFilesLines()));
